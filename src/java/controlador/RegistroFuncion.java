...
public class RegistroFuncion extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String nombrecorto = request.getParameter("txtNombreCorto");
        
        if(funcionalidad.equals("") || nombrecorto.equals("")){
            String error = "No puede haber campos avacios";
            request.getSession().setAttribute("error", error);
            request.getRequestDispatcher("error.jsp").forward(request, response);
        }
        
        if(funcionalidad != "" && nombrecorto != ""){            
            FuncionalidadDAO fun = new FuncionalidadDAO();
            boolean verifica = fun.create(new FuncionalidadDTO(0, nombrecorto, nombrecorto));
