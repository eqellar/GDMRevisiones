 ...
 * @author Corei3
 */
public class ConsultaRev extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String alias = "";
        String vendedor = "";
        String select = request.getParameter("clientes");
        System.out.println(select);

        if (select != null) {
            idcliente = Integer.parseInt(select);
            for (int i = 0; i < listclie.size(); i++) {
                alias = listclie.get(i).getAlias();
                if (select == alias) {
                    idcliente = listclie.get(i).getId_cliente();

                    System.out.println("Aquí tenemos el ID del cliente: " + idcliente);
                }
            }
        }
        ...
        for (int i = 0; i < listdis.size(); i++) {
            ...
            ver = request.getParameter(num); // mando a traer el parametro num y esto se realiza hasta que el id de la solicitud coincide con la variable i del recorrido del for
            ...
            nombre = datoCliente.getNombre_clie();//Capturo el nombre del cliente en la variable nombre
            ...
            request.getSession().setAttribute("nombre", nombre);//Creo una sesión llamada "nombre" con el valor de la variable nombre
            ...
            System.out.println("Datos del cliente" + nombre + ", " + alias + ", " + vendedor); //Prueba de impresión para verificar los valores capturados  
            ...
            request.getRequestDispatcher("detallesRev.jsp").forward(request, response);//Despacho las sesiones antes creadas al jsp detalles.
